...
module Typed : sig
  ...
  val dims : 'a t -> int iarray
  val shape : 'a t -> Shape.t
  val num_dims : 'a t -> int
  val length : 'a t -> int
  val item : 'a t -> 'a
  val get : 'a t -> int iarray -> 'a
  val set : 'a t -> int iarray -> 'a -> unit
  val fill : 'a t -> 'a -> unit
  val left_slice : 'a t -> indices:int iarray -> 'a t
  ...
  val create : 'a Type.t -> dims:int iarray -> 'a -> 'a t
  val init : 'a Type.t -> dims:int iarray -> f:(int iarray -> 'a) -> 'a t
  val zeros : dims:int iarray -> float t
  val ones : dims:int iarray -> float t
  val arange : int -> float t
  val map : 'b Type.t -> 'a t -> f:('a -> 'b) -> 'b t
  val mapi : 'b Type.t -> 'a t -> f:(int iarray -> 'a -> 'b) -> 'b t
  val map2 : 'c Type.t -> 'a t -> 'b t -> f:('a -> 'b -> 'c) -> 'c t
  val iter : 'a t -> f:('a -> unit) -> unit
  val iteri : 'a t -> f:(int iarray -> 'a -> unit) -> unit
...
val get_exn : 'a Type.t -> t -> int iarray -> 'a
val left_slice : t -> indices:int iarray -> t
val sub_left : t -> pos:int -> len:int -> t
val init : 'a Type.t -> dims:int iarray -> f:(int iarray -> 'a) -> t
val of_list : 'a Type.t -> 'a list -> t
val of_list2_exn : 'a Type.t -> 'a list list -> t
val of_lit : 'a Type.t -> 'a -> t
val zeros : dims:int iarray -> t
